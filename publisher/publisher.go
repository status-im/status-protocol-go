...
type Publisher struct {
	...
	protocol    *encryption.ProtocolService
	persistence Persistence
	filter      *filter.ChatsManager
	quit        chan struct{}
	ticker      *time.Ticker
}
...
func (p *Publisher) Init(db *sql.DB, protocol *encryption.ProtocolService, onNewMessagesHandler func([]*filter.Messages)) error {
	filterService, err := filter.New(p.whisper, filter.NewSQLitePersistence(db), onNewMessagesHandler)
	if err != nil {
		return err
	}
...
func (p *Publisher) ProcessPublicBundle(myIdentityKey *ecdsa.PrivateKey, bundle *encryption.Bundle) ([]*multidevice.Installation, error) {
	if p.protocol == nil {
...
func (p *Publisher) GetBundle(myIdentityKey *ecdsa.PrivateKey) (*encryption.Bundle, error) {
	if p.protocol == nil {
		return nil, errProtocolNotInitialized
	}
...
func (p *Publisher) GetPublicBundle(identityKey *ecdsa.PublicKey) (*encryption.Bundle, error) {
	if p.protocol == nil {
		return nil, errProtocolNotInitialized
	}
...
func (p *Publisher) LoadFilters(chats []*filter.Chat) ([]*filter.Chat, error) {
	secrets, err := p.protocol.GetSharedSecretService().All()
	if err != nil {
		return nil, err
	}

	var negotiatedSecrets []filter.NegodiatedSecret

	for _, secret := range secrets {
		negotiatedSecrets = append(negotiatedSecrets, filter.NegodiatedSecret{
			PublicKey: secret.Identity,
			Key:       secret.Key,
		})
	}

	return p.filter.InitDeprecated(chats, negotiatedSecrets)
}
...
func (p *Publisher) ProcessNegotiatedSecret(secrets []*sharedsecret.Secret) {
	for _, secret := range secrets {
		_, err := p.filter.LoadNegotiated(filter.NegodiatedSecret{
			PublicKey: secret.Identity,
			Key:       secret.Key,
		})
		if err != nil {
...
func (p *Publisher) CreateDirectMessage(privateKey *ecdsa.PrivateKey, publicKey *ecdsa.PublicKey, DH bool, payload []byte) (*whisper.NewMessage, error) {
	...
	var (
		msgSpec *encryption.ProtocolMessageSpec
		err     error
	)
...
func (p *Publisher) directMessageToWhisper(myPrivateKey *ecdsa.PrivateKey, theirPublicKey *ecdsa.PublicKey, spec *encryption.ProtocolMessageSpec) (*whisper.NewMessage, error) {
	...
	} else if spec.PartitionedTopic() == encryption.PartitionTopicV1 {
		// p.log.Debug("Sending on partitioned topic", "public-key", destination)
		// Create filter on demand
		if _, err := p.filter.LoadPartitioned(theirPublicKey); err != nil {
			return nil, err
		}
...
func (p *Publisher) CreatePublicMessage(privateKey *ecdsa.PrivateKey, chatID string, payload []byte, wrap bool) (*whisper.NewMessage, error) {
	...
	filter := p.filter.ChatByID(chatID)
	if filter == nil {
...
func (p *Publisher) sendContactCode() (*whisper.NewMessage, error) {
	...
	message, err := p.CreatePublicMessage(privateKey, filter.ContactCodeTopic(&privateKey.PublicKey), nil, true)
	if err != nil {
