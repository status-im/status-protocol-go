...
func (u *MembershipUpdate) extractFrom() error {
	...
	u.From = types.EncodeHex(ecrypto.FromECDSAPub(publicKey))
	return nil
}
...
func groupChatID(creator *ecdsa.PublicKey) string {
	return uuid.New().String() + "-" + types.EncodeHex(ecrypto.FromECDSAPub(creator))
}
...
func (g *Group) ProcessEvent(from *ecdsa.PublicKey, event MembershipUpdateEvent) error {
	fromHex := types.EncodeHex(ecrypto.FromECDSAPub(from))
...
func publicKeyToString(publicKey *ecdsa.PublicKey) string {
	return types.EncodeHex(ecrypto.FromECDSAPub(publicKey))
}
