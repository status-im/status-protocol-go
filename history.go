...
func syncedToOpts(histories []History, now time.Time) statusproto.RequestOptions {
	opts := statusproto.RequestOptions{
		To:    now.Unix(),
		Limit: 1000,
	}
	for i := range histories {
...
func notsyncedToOpts(histories []History, now time.Time) statusproto.RequestOptions {
	opts := statusproto.DefaultRequestOptions()
	opts.To = now.Unix()
	for i := range histories {
		// TODO(dshulyak) remove contact type validation in that function
		// simply always add topic and (if set) public key
