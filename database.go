...
func (db sqliteDatabase) SaveContacts(contacts []Contact) (err error) {
	var (
		tx   *sql.Tx
		stmt *sql.Stmt
	)
...
func (db sqliteDatabase) Contacts() ([]Contact, error) {
...
func (db sqliteDatabase) ContactExist(c Contact) (exists bool, err error) {
	err = db.db.QueryRow("SELECT EXISTS(SELECT id FROM user_contacts WHERE id = ?)", contactID(c)).Scan(&exists)
	return
}

func (db sqliteDatabase) PublicContactExist(c Contact) (exists bool, err error) {
	var pkey []byte
	if c.PublicKey != nil {
...
func (db sqliteDatabase) LastMessageClock(c Contact) (int64, error) {
...
func (db sqliteDatabase) SaveMessages(c Contact, messages []*protocol.Message) (last int64, err error) {
	var (
		tx   *sql.Tx
		stmt *sql.Stmt
	)
...
func (db sqliteDatabase) Messages(c Contact, from, to time.Time) (result []*protocol.Message, err error) {
	contactID := fmt.Sprintf("%s:%d", c.Name, c.Type)
...
func (db sqliteDatabase) NewMessages(c Contact, rowid int64) ([]*protocol.Message, error) {
...
func (db sqliteDatabase) UnreadMessages(c Contact) ([]*protocol.Message, error) {
	contactID := contactID(c)
