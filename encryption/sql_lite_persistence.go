...
func (s *SQLLitePersistence) Open(path string, key string) error {
	db, err := storage.Open(path, key, storage.KdfIterationsNumber)
	if err != nil {
		return err
	}
...
func (s *SQLLitePersistence) GetAnyPrivateBundle(myIdentityKey []byte, installations []*multidevice.Installation) (*BundleContainer, error) {
...
func (s *SQLLitePersistence) GetPublicBundle(publicKey *ecdsa.PublicKey, installations []*multidevice.Installation) (*Bundle, error) {
