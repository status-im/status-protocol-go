...
type Message struct {
	...
	message   interface{}      // decoded message, it's interface{} because it might be of any supported type
...
func (m *Message) Encode(enc Encoder) (err error) {
	m.decryptedPayload, err = enc.Encode(m.message)
	return
}

func (m *Message) Decode(dec Encoder) (err error) {
	m.message, err = dec.Decode(m.decryptedPayload)
	return
}
...
func (m *Message) Send(sender Sender) (err error) {
	if m.encryptionMeta.spec != nil {
		if m.transportMeta.public {
			m.transportMeta.hash, err = sender.SendPublic(m.transportMeta.chatID, m.encryptionMeta.spec)
		} else {
			m.transportMeta.hash, err = sender.SendPrivate(m.encryptionMeta.spec)
		}
	} else if m.transportMeta.public {
		m.transportMeta.hash, err = sender.SendPublicRaw(m.transportMeta.chatID, m.RawMessage())
	} else {
